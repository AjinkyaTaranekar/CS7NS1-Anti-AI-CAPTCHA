...
ATTACKERS_DIR="$REPO_ROOT/attackers"
ATTACKER1="$REPO_ROOT/attackers/attacker1.py"
ATTACKER2="$REPO_ROOT/attackers/attacker2.py"
ATTACKER3="$REPO_ROOT/attackers/attacker3.py"
ATTACKER4="$REPO_ROOT/attackers/attacker4.py"
...
# List of attacker scripts to run sequentially (edit as needed)
ATTACKER_SCRIPTS=(
  "$ATTACKERS_DIR/attacker_1.py"
  "$ATTACKERS_DIR/attacker_2.py"
  "$ATTACKERS_DIR/attacker_3.py"
...
REQUIRED_ENVS=(API_KEY LLM_MODEL)
missing_envs=()
...
if [ ${#missing_envs[@]} -ne 0 ]; then
  echo -e "\n\033[1;31m⚠️  Missing required environment variables: ${missing_envs[*]}\033[0m"
  if [ -f "$ATTACKERS_DIR/.env" ]; then
    echo -e "\nℹ️  A .env file was found at $ATTACKERS_DIR/.env — you can load it by running:\n  source $ATTACKERS_DIR/.env\n"
  fi
  echo -e "\nPlease export the missing variables before running the attackers. Example:\n\n  export API_KEY=your_api_key_here\n  export LLM_MODEL=your_model_here\n\nOr create $ATTACKERS_DIR/.env with the following content:\n  API_KEY=your_api_key_here\n  LLM_MODEL=your_model_here\n\nThen re-run this script. Exiting.\n"
  exit 2
fi

# Optional: GEMINI_API_KEY (attacker_1 uses a hard-coded key in main() by default)
if [ -z "${GEMINI_API_KEY-}" ]; then
  echo -e "\n⚠️  Note: GEMINI_API_KEY is not set. `attacker_1.py` currently contains a hard-coded key in its __main__ section — consider setting GEMINI_API_KEY to override or removing the hard-coded value if you care about security."
fi
for script in "${ATTACKER_SCRIPTS[@]}"; do
  if [ -f "$script" ]; then
    name=$(basename "$script")
    echo -e "\n\033[1;32m▶️ Starting attacker: $name\033[0m"
    "$PY_CMD" "$script" >"$LOG_DIR/attackers/${name%.py}.log" 2>&1 || echo "WARNING: $name returned non-zero exit code, see $LOG_DIR/attackers/${name%.py}.log"
    echo -e "\n\033[1;34m✓ Finished $name — log at $LOG_DIR/attackers/${name%.py}.log\033[0m"
  else
    echo "WARNING: Attacker script not found: $script (skipping)"
  fi
done
